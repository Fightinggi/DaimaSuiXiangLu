...
public:
    TreeNode* trimBST(TreeNode* root, int low, int high) {
        ...
        //如果root的val小于low，将root的右孩子返回到上一层....就是再往root右搜索,,,root左边的子树都不要了
        if (root->val < low) return trimBST(root->right, low, high);
        //如果root的val大于low，将root的左孩子返回到上一层....就是再往root左搜索,,,root左边的子树都不要了
        if (root->val > high) return trimBST(root->left, low, high);
        //上一层的操作
        root->left = trimBST(root->left, low, high);  //把符合的左子树连接
...
void test()
{
    ...
    TreeNode* res = so->trimBST(root, 2,4);  //修剪二叉树
