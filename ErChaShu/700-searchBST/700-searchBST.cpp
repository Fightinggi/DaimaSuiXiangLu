...
public:
    TreeNode* searchBST(TreeNode* root, int val) {
        ...
        //终止条件
        if (root == NULL || root->val == val) return root;  //找到符合的根节点，回溯到上一层函数内，result = root；result=result
        //返回值
        TreeNode* result = NULL;  //没有返回NULL
        //单层逻辑
        if (val < root->val) result = searchBST(root->left, val);//往左子树搜索
        if (val > root->val) result = searchBST(root->right, val);//往右子树搜索
        return result;
...
void test()
{
    ...
    TreeNode* res = so->searchBST(root,1);  //最大二叉树
