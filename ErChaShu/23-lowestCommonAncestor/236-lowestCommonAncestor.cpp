...
public:
    ...
    //从下往上处理节点，后序，左右中，回溯
    //中是处理逻辑的，返回结果
    //左右递归的处理，意思是是否遇到了p，q，如果遇到了，就在中的时候处理，返回中节点root。

    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {

        if (root == NULL) return NULL;
        if (root == p || root == q) return root; //情况二

        TreeNode* left = lowestCommonAncestor(root->left, p, q); //左子树left
        TreeNode* right = lowestCommonAncestor(root->right, p, q); //右子树right

        //处理中root逻辑
        if (left != NULL && right != NULL) return root;
        else if (left != NULL && right == NULL) return left;  //left 也可能是root
        else if (left == NULL && right != NULL) return right; //right 也可能是root
        else return NULL;
...
void test()
{
    ...
    vector<int> res = so->lowestCommonAncestor(root);  //最大二叉树
